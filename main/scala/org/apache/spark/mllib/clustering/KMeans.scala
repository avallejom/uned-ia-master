...
// AVM euclidean or cosine distance 
 private final val distanciaEuclidea = false ;
   ...
   * [[org.apache.spark.mllib.util.MLUtils#fastSquaredDistance]].
   */
  private[clustering] def euclideanDistance(
      v1: VectorWithNorm,
      v2: VectorWithNorm): Double = {
  ...
  /**
   * AVM Returns the cosine distance between two vectors.
   */
  private[clustering] def cosineDistance(
       v1: VectorWithNorm,
       v2: VectorWithNorm): Double = {
    1 - v1.vector.toBreeze.dot(v2.vector.toBreeze) / (v1.norm * v2.norm)
  }
}
